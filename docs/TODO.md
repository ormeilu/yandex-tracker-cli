...
- A Homebrew formula, generated by the release workflow from the tarballs it
  just published. The push step is skipped unless a `TAP_TOKEN` secret exists,
  because a tap lives in a second repository and a release must not fail over an
  optional channel.
